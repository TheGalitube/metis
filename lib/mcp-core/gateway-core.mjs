...
import { corsAllowOrigin, corsAllowedOrigins, requestAuthorization } from "./http-auth.mjs";
import {
  HOST_ONLY_MCP_IDS,
  assertHostOnlyMcpMutationAllowed,
...
import { isDockerEnv, rewriteDockerServiceUrl } from "../docker-urls.mjs";
...
const LISTEN_HOST = env("MCP_HOST", isDockerEnv() ? "0.0.0.0" : "127.0.0.1");
...
const INTERNAL_ORIGIN = isDockerEnv()
  ? rewriteDockerServiceUrl(env("AI_CHAT_INTERNAL_ORIGIN") || "http://app:3100", "app", 3100)
  : env("AI_CHAT_INTERNAL_ORIGIN", env("AI_CHAT_PUBLIC_URL"));
function configuredInternalUrl(name, route) {
  const configured = env(name);
  if (configured) return rewriteDockerServiceUrl(configured, "app", 3100);
  if (!INTERNAL_ORIGIN) {
    throw new Error(`${name} is not configured; set the explicit Metis internal runtime URL`);
  }
  return rewriteDockerServiceUrl(`${INTERNAL_ORIGIN.replace(/\/+$/, "")}${route}`, "app", 3100);
}
...
const configuredInteractiveWaitMs = Number.parseInt(
  env("MCP_INTERACTIVE_WAIT_TIMEOUT_MS", String(30 * 60_000)),
  10,
);
const INTERACTIVE_WAIT_TIMEOUT_MS = Number.isFinite(configuredInteractiveWaitMs)
  ? Math.max(60_000, Math.min(configuredInteractiveWaitMs, 24 * 60 * 60_000))
  : 30 * 60_000;
...
const OWNER_SCOPED_MCP_TOOLS = new Set([
  "upsert_mcp_server",
  "set_mcp_server_enabled",
  "provision_registry_server",
]);
...
const DEFAULT_REGISTRY = [
  ...
  jarvisBridgeRegistryEntry(ROOT),
  {
    id: "memory",
...
async function postRuntimeApproval(token, payload, headers, signal) {
...
async function ensureRuntimeApproval(name, args, context = {}, signal) {
  ...
  const status = await fetch(`${INTERNAL_APPROVAL_URL}?chatId=${encodeURIComponent(chatId)}&sessionScope=${encodeURIComponent(sessionScope)}`, {
    headers: {
      Authorization: `Bearer ${token}`,
      "X-AI-Chat-User-Id": String(context.userId || ""),
      "X-AI-Chat-Job-Id": jobId,
  ...
  if (
    statusBody.approvedOnce === true ||
    shouldAutoApprove(Array.isArray(statusBody.approvedPatterns) ? statusBody.approvedPatterns : [], name, args)
  ) return;
  ...
  for (;;) {
    ...
    if (pollBody.status === "resolved") {
      ...
      if (pollBody.decision === "allow-session") return;
      if (pollBody.decision === "allow") {
        const consumed = await fetch(
          `${INTERNAL_APPROVAL_URL}?chatId=${encodeURIComponent(chatId)}&sessionScope=${encodeURIComponent(sessionScope)}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              ...headers,
        ...
        if (!consumed.ok || consumedBody.approvedOnce !== true) {
          throw new Error("The one-time runtime approval could not be consumed safely.");
        }
        return;
      }
    }
    if (Date.now() >= deadline) {
      ...
      throw new Error(`Runtime approval timed out after ${Math.round(INTERACTIVE_WAIT_TIMEOUT_MS / 60_000)} minutes.`);
    }
...
function assertToolPolicy(name, context = {}, args = {}) {
  ...
  if (context.incognito && new Set([
    ...
    "update_chat_title", "update_chat_keywords", "search_chats",
  ]).has(name)) {
    throw new Error(`${name} is unavailable in Incognito.`);
  }
  if (OWNER_SCOPED_MCP_TOOLS.has(name) && !String(context.userId || "").trim()) {
    throw new Error(`${name} requires an authenticated account`);
  }
  assertRawHttpBearerToolAllowed(name, context, process.env.MCP_ALLOW_REMOTE_ADMIN === "true");
}
...
function visibleRegistry(registry, context = {}) {
  let servers = context.userId
    ...
    : registry;
  if (context.userId && !isHostAdminContext(context, process.env.MCP_IS_HOST_ADMIN === "1")) {
    servers = servers.filter((entry) => !HOST_ONLY_MCP_IDS.has(entry.id));
  }
  return servers;
}
...
const tools = [
  ...
  },
  {
    ...
    description: "Create a one-time or recurring autonomous agent automation. Every run gets its own persistent chat transcript and can use the same allowed tools, child MCPs, remote tools, and persistent Metis browser as a normal Agent-mode run. Recurring schedules can use minutes, days, or a calendar day each month. modelParams sets Reasoning, Fast, and other model options for the primary model. extendedModelParams does the same for the delegated/extended model. After a successful create, include a Markdown link [name](automation://<id>) using the id from the result. Do not claim creation without a completed tool call.",
    inputSchema: {
      type: "object",
      properties: {
        name: { type: "string" },
        ...
        modelParams: {
          type: "array",
          description: "Model options for each run, such as Reasoning effort, Fast, and other provider parameters (yes/no or enumerated values).",
          items: {
            type: "object",
            properties: {
              id: { type: "string", description: "Parameter id, e.g. effort, fast, context." },
              value: { type: "string", description: "Parameter value, e.g. high, true, false." },
            },
            required: ["id", "value"],
            additionalProperties: false,
          },
        ...
        extendedModelParams: {
          type: "array",
          description: "Reasoning, Fast, and other options for the delegated/extended model.",
          items: {
            type: "object",
            properties: {
              id: { type: "string", description: "Parameter id, e.g. effort, fast, context." },
  ...
  },
  {
    ...
    description: "Update an existing automation's name, prompt, context chat, models, model options, extended model options, maximum run duration, timezone, or schedule.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  },
  {
    ...
    description: "Set a short 2-6 word sidebar title for the current chat. Use a noun phrase, not the user's first prompt. Call this once the topic is clear, and again when the topic changes. Do not mention this in the reply.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "list_memories",
    description: "Retrieve durable memories for the current project when this chat belongs to one; otherwise retrieve account memories.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
    annotations: { readOnlyHint: true },
  },
  {
    name: "add_memory",
    description: "Add a durable memory to the current project when this chat belongs to one; otherwise save it for the account. Only save useful user-approved facts or preferences.",
    inputSchema: {
      type: "object",
      properties: {
        content: { type: "string" },
  ...
  {
    name: "edit_memory",
    description: "Edit one existing memory by id in the current project, or in account memory when no project is active.",
    inputSchema: {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  {
    name: "delete_memory",
    description: "Delete one existing memory by id from the current project, or from account memory when no project is active.",
    inputSchema: {
      type: "object",
...
function createServerForSession(context = {}) {
  ...
  server.setRequestHandler(
    CallToolRequestSchema,
    async (request, extra) =>
      dispatchGatewayTool(request.params.name, request.params.arguments || {}, {
        context,
        signal: extra?.signal,
      }),
  );
...
function visibleToolsForContext(context = {}) {
  ...
  return tools.filter((tool) =>
    capabilityAllowsTool(tool.name, context) &&
    rawHttpBearerToolAllowed(tool.name, context, process.env.MCP_ALLOW_REMOTE_ADMIN === "true"),
  );
}
...
async function dispatchGatewayTool(name, args = {}, options = {}) {
  ...
  const { auditCall = true, context = {} } = options;
    ...
    try {
      ...
      await ensureRuntimeApproval(name, args, context, options.signal);
      if (
      ...
      if (name === "ask_user") {
        ...
        const response = await fetch(baseUrl, {
          ...
          signal: requestSignal(options.signal, INTERACTIVE_WAIT_TIMEOUT_MS + 60_000),
        });
        const body = await response.text();
      ...
      if (name === "request_mode_change") {
        ...
        }, { context, auditCall: false, signal: options.signal });
      ...
      if (name === "upsert_mcp_server") {
        if (!String(context.userId || "").trim()) throw new Error("upsert_mcp_server requires an authenticated account");
        assertHostOnlyMcpMutationAllowed(args.id, context, process.env.MCP_IS_HOST_ADMIN === "1");
        ...
        const registry = await loadRegistry();
        const localHostAdmin = process.env.MCP_IS_HOST_ADMIN === "1";
        const shared = registry.findIndex((s) => s.id === args.id && !s.ownerId);
        const owned = registry.findIndex((s) => s.id === args.id && s.ownerId === context.userId);
        if (shared >= 0) assertMcpServerMutationAllowed(registry[shared], context, localHostAdmin);
        const existing = shared >= 0 ? shared : owned;
        const entry = { ...(existing >= 0 ? registry[existing] : {}), ...args, ownerId: context.userId };
        assertMcpServerMutationAllowed(entry, context, localHostAdmin);
        if (shared >= 0 || HOST_ONLY_MCP_IDS.has(args.id)) delete entry.ownerId;
      ...
      if (name === "set_mcp_server_enabled") {
        if (!String(context.userId || "").trim()) throw new Error("set_mcp_server_enabled requires an authenticated account");
        assertHostOnlyMcpMutationAllowed(args.server, context, process.env.MCP_IS_HOST_ADMIN === "1");
        const registry = await loadRegistry();
        const entry = registry.find((s) => s.id === args.server && s.ownerId === context.userId)
          || registry.find((s) => s.id === args.server && !s.ownerId);
        if (!entry) throw new Error(`Unknown MCP server: ${args.server}`);
        assertMcpServerMutationAllowed(entry, context, process.env.MCP_IS_HOST_ADMIN === "1");
...
function applyGatewayCors(req, res) {
  const allowedOrigin = corsAllowOrigin(req.headers.origin, corsAllowedOrigins({
    internalOrigin: INTERNAL_ORIGIN,
    extra: env("MCP_CORS_ORIGIN"),
  }));
  if (allowedOrigin) {
...
async function main() {
  ...
  const httpServer = http.createServer(async (req, res) => {
    ...
    if (url.pathname === "/registry" && req.method === "GET") {
      const context = auth.trustedContext || { transport: "http" };
      if (!rawHttpBearerToolAllowed("list_mcp_servers", context, process.env.MCP_ALLOW_REMOTE_ADMIN === "true")) {
  ...
  httpServer.listen(PORT, LISTEN_HOST, () => console.log(`[mcp] ${APP_NAME} Universal MCP Gateway listening on http://${LISTEN_HOST}:${PORT}/mcp`));

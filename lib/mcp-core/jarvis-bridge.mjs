    ...
    try {
      ...
      try {
        response = await fetchImpl(new URL(route, config.url), {
          method, headers, body, signal: controller.signal, redirect: "error", cache: "no-store",
        });
      } catch {
        throw new Error(controller.signal.aborted ? "Jarvis request timed out" : "Jarvis core is unavailable");
      }
      if (!response.ok) {
      ...
      try {
        ...
        if (controller.signal.aborted) throw new Error("Jarvis request timed out");
        throw error;
      }
    } finally {
